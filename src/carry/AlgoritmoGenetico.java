        ...
        for (Individuo individuo : populacao) {
            for (int i = 0; i < individuo.caminho.length; i++) {
                ...
                for (int j = 0; j < individuo.caminho[i].length; j++) {
                    saida += individuo.caminho[i][j];
                    chegada += individuo.caminho[j][i];
                }

                individuo.violacao += Math.abs(1 - saida);
                individuo.violacao += Math.abs(1 - chegada);
            }

            boolean[] passada = new boolean[GENES];
            for (int i = 0; i < passada.length; i++) {
                for (int j = 0; j < individuo.cromossomo.length; j++) {
                    if (individuo.cromossomo[j] == i) {
                        passada[i] = true;
                    }
                }
            }
    ...
    private static void funcaoObjetivo(Individuo[] populacao) {
        ...
        for (Individuo individuo : populacao) {
            for (int i = 0; i < individuo.cromossomo.length - 1; i++) {
                individuo.fitness += CUSTO[individuo.cromossomo[i]][individuo.cromossomo[i + 1]] * individuo.caminho[individuo.cromossomo[i]][individuo.cromossomo[i + 1]];
            }
